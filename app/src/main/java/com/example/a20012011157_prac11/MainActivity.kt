...
import
com.example.notes_mad_prac_11_20012011169.databinding.ActivityMainBinding
import
com.example.notes_mad_prac_11_20012011169.databinding.NoteEditViewBinding
import java.util.ArrayList
class MainActivity : AppCompatActivity() {
    private lateinit var binding: ActivityMainBinding
    private val TAG = "MainActivity"
    private var listener: ((note: Note, baseListAdapter: NotesAdapter,
                            mode: NoteMode, position: Int)->Unit)? =
        { note: Note, _: NotesAdapter, noteMode: NoteMode, pos: Int ->
            note.modifiedTime = Note.getCurrentDateTime()
            if (noteMode == NoteMode.add) {
                if (!createNote(note))
                    Toast.makeText(this, "Enter Valid Note",
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.fab.setOnClickListener {
            showAlertDialog(
                ...
                Note("", "", "", Note.getCurrentDateTime()), -1,
                notesRecycleAdapter
            )
        }
    }
    ...
    fun showAlertDialog(
        ...
        val binding =
            NoteEditViewBinding.inflate(LayoutInflater.from(this))
        ...
        builder.setPositiveButton(
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
                ...
                newNote.remindertime =
                    Note.getMillis(binding.reminderTime.currentHour,
